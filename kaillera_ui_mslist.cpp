	...
	void refresh_servers(){
		...
		if (runn) {
			...
			while (runn && (ptr + 30 < end || plist.size() > 0)) { 
				...
				//xxx.xx.stt = st[xx.z++];
				wsprintf(xxx.xxz, "pinged %i servers", total);//wsprintf(xxx.xxz, "Refreshing List... [C: %i, T: %i, P: %i, F: %i]", k_socket::list.size(), total, plist.size(), k_socket::sockets.fd_count);
			...
			SetWindowText(kaillera_mslref,xxx.xxz);
		}  else SetWindowText(kaillera_mslref,"Error refreshing list");
	}
	void run() {
		runn = running = true;
		__try {
	...
	void refresh_wgames(){
		...
		if (runn) {
			...
			while (runn && (ptr + 30 < end || plist.size() > 0)) { 
				...
				wsprintf(xxx.xxz, "pinged %i games", total);
				SetWindowText(kaillera_mslref, xxx.xxz);
				PingSocket::step(10);
				DWORD ti = GetTickCount();
				for (int _x = 0; _x < plist.size(); _x++) {
			...
			wsprintf(xxx.xxz, "%i games found", total);
			SetWindowText(kaillera_mslref,xxx.xxz);
		} else SetWindowText(kaillera_mslref,"Error refreshing list");
	...
	void refresh_p2p_wgames() {
		...
		if (runn) {
			...
			if (runn) {
				...
				wsprintf(xxx.xxz, "%i waiting games found", total);
				SetWindowText(kaillera_mslref,xxx.xxz);
			}
		}
	}
