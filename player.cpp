...
void RecordsList_Populate_fn(char * fn, int i) {
	...
	PlayBackBuffer.ptr = PlayBackBuffer.buffer;
	PlayBackBuffer.end = PlayBackBuffer.buffer + len;
	_lclose(in);
	char VER[4];
	...
	sprintf(filename, "%02d:%02d - %02d/%02d",ecx->tm_hour,ecx->tm_min, ecx->tm_mday, ecx->tm_mon);
	...
	case WM_INITDIALOG:
		{
			...
			RecordsListDlg_list.AddColumn("Game", 250);
			RecordsListDlg_list.AddColumn("App", 160);
			RecordsListDlg_list.AddColumn("Time", 80);
			RecordsListDlg_list.AddColumn("fn", 0);
			RecordsListDlg_list.AddColumn("Length", 60);
